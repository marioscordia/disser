...
The included studies span a range of publication venues, with MDPI journals (\textit{Sensors}, \textit{Applied Sciences}, \textit{Electronics}, \textit{Information}) accounting for the largest share, followed by IEEE Access, Elsevier, and Springer venues. Of the 50 studies, 10 (20\%) directly address safety violation detection in educational institutions or involve child-specific analysis; the remaining 38 (76\%) focus on general public-space surveillance but were retained for methodological transferability. This distribution underscores the need for dedicated child-focused datasets and benchmarks (see Section~4.6).
...
Fig.~\ref{fig:keyword-network} illustrates the keyword co-occurrence network. The term ``deep learning'' exhibits the highest frequency (23 occurrences) and serves as the central hub, with ``violence detection'' (19) and ``anomaly detection'' (16) forming the primary application nodes, and ``convolutional neural network'' (15), ``CNN'' (15), and ``LSTM'' (10) constituting the dominant architectural terms.

\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{VOSViewer/vosviewer_keyword_network.png}}
...
The temporal overlay (Fig.~\ref{fig:overlay}) shows attention mechanisms, YOLO-based architectures, and transformer-integrated approaches concentrated in 2025--2026, whereas earlier publications (2022--2023) employ autoencoder-based reconstruction and traditional 3D CNN frameworks.

\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{VOSViewer/vosviewer_overlay.png}}
\caption{Keyword overlay visualization. Keywords colored by average publication year from blue (2022--2023) to yellow (2025--2026).}
\label{fig:overlay}
\end{figure}
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{Citespace/citespace_burst_timeline.png}}
\caption{Top 10 keywords with the strongest citation bursts. Red segments indicate periods of heightened citation activity; ``attention mechanism'' and ``transfer learning'' exhibit the most pronounced recent bursts (2025--2026).}
...
Several limitations pervade the reviewed literature. First, dominant benchmarks (UCF-Crime, ShanghaiTech Campus, Hockey Fights) feature adult subjects in non-educational settings, limiting transferability to schools~\cite{b15}; performance figures herein indicate benchmark capability, not readiness for deployment. Second, exclusive reliance on accuracy/AUC obscures false alarm rates, latency, and environmental robustness. Third, only two of 50 papers address privacy despite monitoring minors~\cite{b11}. Fourth, geographic concentration in South/East Asia limits environmental and behavioral diversity. Fifth, studies are evaluated in isolation from school workflows. The evidence base reflects moderate venue rigor: MDPI journals account for approximately half the corpus, with no papers from top-tier CV venues (CVPR, ICCV, ECCV), confirming this remains a niche application.
...
\item \textbf{Cross-domain robustness:} Testing across schools, camera types, and demographics should become standard evaluation protocol.
\item \textbf{Human-in-the-loop integration:} Edge-detected alerts should route to centralized dashboards with tiered prioritization (weapon > fight > suspicious behavior) and confidence thresholds to prevent notification fatigue; research on alert management interfaces for educational contexts remains open.
\end{enumerate}
