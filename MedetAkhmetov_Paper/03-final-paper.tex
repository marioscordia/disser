...
\begin{figure}[t]
\centerline{\includegraphics[width=0.55\columnwidth]{Prisma/prisma_diagram.png}}
...
Keywords were extracted from titles and abstracts using a curated vocabulary of 30 terms organized into four categories: \textit{Architecture}---CNN, LSTM, GRU, ResNet, MobileNet, 3D CNN, YOLO, autoencoder; \textit{Temporal/Motion}---optical flow, spatiotemporal features, action recognition, behavior recognition, skeleton-based; \textit{Application/Deployment}---violence detection, anomaly detection, weapon detection, firearm detection, video surveillance, CCTV, real-time detection, IoT, edge computing, crowd analysis; \textit{Methods/Cross-cutting}---deep learning, attention mechanism, transfer learning, pose estimation, feature extraction, convolutional neural network, object detection. Co-occurrence matrices were constructed to model the frequency with which keyword pairs appeared within the same paper. ``Convolutional neural network'' and ``CNN'' were treated as separate entries to capture papers using the abbreviated versus full form, but were conceptually grouped for cluster interpretation; similarly, ``video surveillance'' and ``CCTV'' were retained as distinct terms with different connotations.
...
The included studies span a range of publication venues, with MDPI journals accounting for the largest share, followed by IEEE Access, Elsevier, and Springer venues. Of the 50 studies, 10 (20\%) directly address safety violation detection in educational institutions or involve child-specific analysis; the remaining 40 (80\%) focus on general public-space surveillance but were retained for methodological transferability. This distribution underscores the need for dedicated child-focused datasets and benchmarks (see Section~4.6).

\subsection{Publication Trends}

Scopus contributed 44 of the 50 reviewed studies (88\%), with Web of Science accounting for the remaining 6 (12\%). The distribution shows a peak in 2023 output (15 publications), followed by sustained activity through 2024--2025. Although Google Scholar contributed 94 records to the initial search pool, none survived to the final corpus after duplicate removal---reflecting substantial overlap with Scopus and Web of Science results---and subsequent eligibility screening; its role was therefore limited to broadening the initial search net.

\subsection{Keyword Co-occurrence Analysis}

Fig.~\ref{fig:vosviewer} illustrates the keyword co-occurrence network. ``Deep learning'' (23 occurrences) is the central hub, with ``violence detection'' (19) and ``anomaly detection'' (16) as primary application nodes, and ``convolutional neural network'' (15), ``CNN'' (15), and ``LSTM'' (10) as the dominant architectural terms.

\begin{figure*}[t]
\centerline{\includegraphics[width=\textwidth]{VOSViewer/vosviewer_keyword_network.png}}
\vspace{2pt}
\centerline{\includegraphics[width=0.485\textwidth]{VOSViewer/vosviewer_overlay.png}\hfill
\includegraphics[width=0.485\textwidth]{VOSViewer/vosviewer_density.png}}
\caption{(a) Keyword co-occurrence network --- node size reflects keyword frequency, edges represent co-occurrence links; (b) temporal overlay --- keywords colored by average publication year from blue (2022--2023) to yellow (2025--2026); (c) keyword density visualization --- warmer (yellow/red) regions indicate higher density of co-occurrence activity.}
\label{fig:vosviewer}
\end{figure*}

The temporal overlay (panel b) shows attention mechanisms, YOLO-based architectures, and transformer-integrated approaches concentrated in 2025--2026, whereas earlier publications (2022--2023) employ autoencoder-based reconstruction and traditional 3D CNN frameworks.

Top keywords by frequency are listed in Table~\ref{tab:keywords}.

\begin{table}[t]
\caption{Top 10 Keywords by Frequency}
\begin{center}
\footnotesize
...
\hline
\textbf{Rank} & \textbf{Keyword} & \textbf{Freq.} & \textbf{Category} \\
\hline
1 & Deep learning & 23 & Methods/Cross-cutting \\
2 & Violence detection & 19 & Application/Deployment \\
3 & Anomaly detection & 16 & Application/Deployment \\
4 & Convolutional neural network & 15 & Architecture \\
5 & CNN & 15 & Architecture \\
6 & Video surveillance & 14 & Application/Deployment \\
7 & IoT & 12 & Application/Deployment \\
8 & CCTV & 11 & Application/Deployment \\
9 & LSTM & 10 & Architecture \\
10 & Object detection & 9 & Methods/Cross-cutting \\
\hline
\end{tabular}
\label{tab:keywords}
\end{center}
...
The burst analysis shows ``attention mechanism'' and ``transfer learning'' with pronounced burst activity in 2025--2026, while ``CNN'' and ``LSTM'' show sustained presence across 2022--2026 as foundational techniques.

\begin{figure*}[t]
\centerline{\includegraphics[width=\textwidth]{Citespace/citespace_burst_timeline.png}}
\vspace{2pt}
\centerline{\includegraphics[width=0.9\textwidth]{Citespace/citespace_trend.png}}
\caption{(a) Top 10 keywords with the strongest citation bursts --- red segments indicate periods of heightened citation activity; ``attention mechanism'' and ``transfer learning'' exhibit the most pronounced recent bursts (2025--2026); (b) annual publication trend by database (2022--2026), showing Scopus as the dominant source.}
\label{fig:citespace}
\end{figure*}
...
The four thematic clusters reveal a field organized around a central tension: architectural sophistication versus deployability. C1 (Architectures) emphasizes accuracy maximization through increasingly complex networks; C3 (Application and Deployment) privileges real-time throughput and hardware efficiency via lightweight YOLO and MobileNet variants. This tension is visible in the temporal evolution: the overlay visualization shows a transition from autoencoder-based reconstruction (2022--2023) to YOLO and transformer-integrated approaches (2025--2026), reflecting a field moving toward architectures specifically adapted for surveillance while simultaneously addressing edge-deployment constraints. The CNN--LSTM paradigm, observed in 26 of 50 studies, indicates consensus that effective video anomaly detection requires integrating spatial feature extraction with temporal sequence modeling; the recent growth in attention mechanisms suggests movement beyond naive concatenation toward more sophisticated feature weighting~\cite{b5,b17}.

\subsection{Cross-Cluster Comparative Analysis}

The four clusters share deep learning as their core paradigm---every reviewed study depends on convolutional or recurrent architectures for feature representation. C2 (Temporal and Motion) and C4 (Methods and Techniques) function as methodological bridges: C2's motion strategies provide the temporal understanding that C1 and C3 depend upon, while C4's transfer learning and attention mechanisms enhance performance across all clusters. Few studies address the intersection of lightweight architectures (C1/C3) with skeleton-based child-specific modeling (C2).

Two productive methodological tensions emerge. First, the accuracy--efficiency trade-off: C1's deep CNN-LSTM ensembles exceed 90\% accuracy (best surpassing 95\%) yet require GPU-class hardware; C3 achieves real-time edge inference at marginally lower accuracy. Second, the role of appearance: C2's motion-centric approaches suppress appearance to avoid bias, while C4's techniques are appearance-driven. Reconciling these through dynamic multimodal fusion remains open.

\subsection{Most Effective Methods}

Hybrid architectures combining 3D or pre-trained 2D CNNs with recurrent units (LSTM, BiLSTM, GRU) and attention mechanisms achieve the strongest performance, with accuracies exceeding 90\% (strongest surpassing 95\%) on controlled benchmarks~\cite{b17,b18,b2}. For weapon detection, YOLOv5 and Scaled-YOLOv4 demonstrate the best accuracy--speed trade-off (mAP $>$90\%) at real-time throughput~\cite{b19,b9}. Skeleton-based approaches using pose estimation show promise for child-specific applications by abstracting away from appearance-based biases~\cite{b16,b20}. Beyond video surveillance, LSTM-based autoencoder architectures have proven effective for anomaly detection in adjacent domains such as software quality monitoring, where they detected critical regressions up to two days before official reporting~\cite{b24}.
...
\item \textbf{Human-in-the-loop integration:} Edge-detected alerts should route to centralized dashboards with tiered prioritization (weapon > fight > suspicious behavior) and confidence thresholds to prevent notification fatigue; research on alert management interfaces for educational contexts remains open; integrated monitoring architectures combining classification and anomaly detection modules~\cite{b24} offer a transferable design framework.
\end{enumerate}
